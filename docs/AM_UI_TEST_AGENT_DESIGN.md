...
flowchart TD
    ...
    subgraph Agent State Loop
        ...
        Assert -- "Pass (Finished)" --> Report["ReportNode (Aggregate Logs & Persist)"]
    end
    ...
    subgraph Storage & External APIs
        ...
        Agent -- "Query Memory" --> Qdrant["Qdrant Vector Store (Port 6333)"]
...
4. **SelfHealNode (`Self-Healer`)**: Triggers when Playwright fails to find a selector. Uses Qwen2.5-VL to locate the button/field on the screenshot, calculates coordinates, executes the click, and updates the selectors database in Qdrant.
5. **ReportNode (`Reporter`)**: Generates an HTML report, saves artifacts to MongoDB, and signals completion.
